...
CREATE TABLE IF NOT EXISTS Exercicio (
  ...
  data_criacao TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
  data_modificacao TIMESTAMP DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
  INDEX idx_nome_exercicio (nome), 
  CONSTRAINT uc_nome_exercicio UNIQUE (nome),
...
('Panturrilha no Leg Press', 'Apoie a ponta dos pés na parte inferior da plataforma e empurre, estendendo os tornozelos.', 'Mantenha os joelhos levemente flexionados e não os trave.', '/gifs/panturrilha_legpress.gif', 14, 8, 1);-- Exclusão de Tabelas (em ordem reversa de dependência)
DROP TABLE IF EXISTS Log;
DROP TABLE IF EXISTS Exercicio;
DROP TABLE IF EXISTS Equipamento;
DROP TABLE IF EXISTS Grupo_muscular;
DROP TABLE IF EXISTS Adm;
DROP TABLE IF EXISTS sessions; -- Adicionado para limpar a tabela de sessões antigas
...
-- Armazena os dados das sessões PHP para persistência na nuvem (Vercel).
-- -----------------------------------------------------
